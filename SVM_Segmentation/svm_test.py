...
for splitnumber, (train_index, test_index) in enumerate(skf.split(X_train, y_train)):
    ...
    f1 = predictScore(X_test_split, y_test_split, w)
    model[splitnumber] = {"train_index": train_index, "test_index": test_index, "w": w, "hist": hist, "f1": f1,
                          "image_size": IMGSIZE}
    print(model[splitnumber]["w"])

f1_mean_model = np.mean([model[i]["f1"] for i in model.keys()])
...
for i in range(Ntest):
    ...
    ax.axis('On')
    ax.set_title(f"Test img: {ii+1} F1:{round(f1_score(gt, pred), 2)}")
